...
`wirebody-mcp` requires Rust/Cargo and OpenSSL development libraries available to Cargo.
...
- **`WIREBODY_TOKEN`** / **`WIREBODY_ROOT`** — the pairing secret shown in Wirebody Settings. It is used locally as the HKDF root for both the Bonjour instance label and the TLS-PSK key.
...
| `Unreachable` | iOS app is closed/backgrounded, server toggle off, discovery failed, or peers are not on the same LAN | Ask the user to open Wirebody and confirm both devices are on the same Wi-Fi |
...
[github.com/alpinevm/wirebody](https://github.com/alpinevm/wirebody). MIT-licensed. Issues and PRs welcome.
